...
class Train(object):
    ...
    def generator(self,name,noise,reuse):
        with tf.variable_scope(name,reuse=reuse):
            ...
            output = deconv2d('g_dcon1',output,5,outshape=[self.batch_size,self.img_size/8,64*4])
            output = tf.nn.relu(self.bn('g_bn2',output))

            output = deconv2d('g_dcon2', output, 5, outshape=[self.batch_size, self.img_size / 4, 64 * 2])
            output = tf.nn.relu(self.bn('g_bn3', output))

            output = deconv2d('g_dcon3', output, 5, outshape=[self.batch_size, self.img_size / 2, 64 * 1])
            output = tf.nn.relu(self.bn('g_bn4', output))

            output = deconv2d('g_dcon4', output, 5, outshape=[self.batch_size, self.img_size , self.dim])
    ...
    def discriminater(self,name,inputs,reuse):
        with tf.variable_scope(name,reuse=reuse):
            output = conv2d('d_con1',input,5,64)
            output = lrelu(self.bn('d_bn1',output))
            output = conv2d('d_con2', input, 5, 64*2)
            output = lrelu(self.bn('d_bn2', output))
            output = conv2d('d_con3', input, 5, 64*4)
            output = lrelu(self.bn('d_bn3', output))
            output = conv2d('d_con4', input, 5, 64*8)
